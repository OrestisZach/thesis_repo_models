...
from .deformable_transformer_final import build_deformable_transformer_final
    ...
    def forward(self, points: List[torch.Tensor]) -> Tuple[List[torch.Tensor], torch.Tensor, torch.Tensor]:
        ...
        for det in results:
            ...
            if b.numel() == 0:
                ...
                yaw = b[:, 6].clone()  # raw detector heading (radians)
            norm, s = self._pad_proposals(norm, s, self.proposal_count)
            prop_boxes.append(norm)
            prop_scores.append(s)
            prop_yaw.append(self._pad_yaw(yaw, self.proposal_count))

        # Expose the per-proposal heading as a side channel (the return tuple is
        # unchanged) so the LIVE `_prepare_pointpillars_batch` path can apply the
        # same SEED rotation / angle refinement it uses on the cached path.
        self.last_prop_yaw = torch.stack(prop_yaw, dim=0)
        return list(feats), torch.stack(prop_boxes, dim=0), torch.stack(prop_scores, dim=0)


class ReferModelLangDec(nn.Module):
    """Single-frame LiDAR referring detector with PointPillars proposal seeding.

    Identical to ``ReferModel`` (models/refer_model.py) in every respect except
    one addition: the decoder layers gain a per-query language cross-attention
    sub-layer (see models/deformable_transformer_final.py). Frozen PointPillars
    proposals, query init, matcher, BCE ``loss_refer`` and all hyperparameters
    ...
    def __init__(self, args):
        ...
        self.transformer = build_deformable_transformer_final(args)
    ...
    def _prepare_pointpillars_batch(self, data: Dict):
        ...
        if cached_srcs is None or cached_props is None:
            ...
            # Live path now carries the proposal heading (side channel), so the
            # rotation / pred_yaw behave exactly as on the cached training path.
            return list(feats), props, self.detector_bridge.last_prop_yaw
        ...
        if needs_detector:
            ...
            miss_yaw = self.detector_bridge.last_prop_yaw
...
def build_lang_dec(args):
    """Build proposal-seeded single-frame LiDAR referring model (FINAL variant).

    Identical to ``refer_model.build`` except the transformer decoder gains a
    per-query language cross-attention sub-layer.
